...
export async function validateWorkspacePath(requestedPath) {
  try {
    ...
    if (normalizedPath === '/') {
      // 特别允许根目录
    } else if (FORBIDDEN_PATHS.includes(normalizedPath)) {
      return {
        valid: false,
    ...
    // Fix for root path (avoiding double slashes like '//anti')
    const rootPrefix = resolvedWorkspaceRoot.endsWith(path.sep) ? resolvedWorkspaceRoot : resolvedWorkspaceRoot + path.sep;
